...
// 设置符合BITMAP内存数据的矩形
void CMmxBase::SetCoverRect(__in CSize DstSize, __inout CRect &SetRect)
{
	if (SetRect.left < 0)
		SetRect.left = 0;
...
// 设置内存指定区域的Alpha值
void CMmxBase::ARGB32_CoverAlpha(__inout BYTE *pbyDst, __in CSize DstSize, __in CRect SetRect, __in BYTE byComA, __in BYTE bySetA)
{
