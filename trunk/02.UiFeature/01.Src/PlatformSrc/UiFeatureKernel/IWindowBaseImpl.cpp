...
//////////////////////////////////////////////////////////////////////////
	...
	// 初始化对话框皮肤
	m_pWndProp = pPropSkinMgr->InitWindowSkin(m_strSkinPath.c_str(), m_strWindowName.c_str());
	if (m_pWndProp == NULL)
	{
		// 向窗口发送通知：初始化皮肤异常
...
void IWindowBaseImpl::OnLButtonDown(int nVirtKey, POINT pt)
{
	...
	if (pControl == NULL)
	{
		...
		// 是否最大化
		if (m_pWndProp->IsFullScreen())
			return;

		// 是否支持鼠标点击移动窗口
		if (m_pWndProp->GetDragWindow())
