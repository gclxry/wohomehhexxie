...
///// 重要消息值，外界程序不能与之重复 /////////////////////////////////////////////////////////////////////
// UI内核内部自定义消息值
#define UM_UI_BASE_DEF_MSG							(WM_APP + 0x0200)
// 需要外部对话框接受的消息：使用皮肤初始化窗口正确结束
#define UM_INIT_WINDOW_SUCCESS						(UM_UI_BASE_DEF_MSG + 0x0001)
// 需要外部对话框接受的消息：使用皮肤初始化窗口异常
#define UM_INIT_WINDOW_ERROR						(UM_UI_BASE_DEF_MSG + 0x0002)
// 自动化测试消息值
#define UM_AUTO_TEST								(UM_UI_BASE_DEF_MSG + 0x0003)
// 控件注册要取得的消息
#define UM_REG_CTRL_MSG								(UM_UI_BASE_DEF_MSG + 0x0004)
// 控件注销要取得的消息
#define UM_UNREG_CTRL_MSG							(UM_UI_BASE_DEF_MSG + 0x0005)
// IE控件专用的消息
#define UM_IE_MSG									(UM_UI_BASE_DEF_MSG + 0x0006)
// flash控件专用的消息
#define UM_FLASH_MSG								(UM_UI_BASE_DEF_MSG + 0x0007)
// EDIT控件专用的消息
#define UM_EDIT_MSG									(UM_UI_BASE_DEF_MSG + 0x0008)
// 系统发送过来的SYS_COMMAND消息的拦截，转发给应用程序，目前会将 SC_CLOSE 消息拦截并派发给窗口
#define UM_SYS_COMMAND								(UM_UI_BASE_DEF_MSG + 0x0009)
// 播放控件动画
#define UM_CTRL_ANIMATION							(UM_UI_BASE_DEF_MSG + 0x000A)
// 窗口专用消息
#define UM_WINDOWS									(UM_UI_BASE_DEF_MSG + 0x000B)
// 弹出任务栏菜单消息，内核会拦截这个消息，同时将这个消息转发给对话框，对话框可以自由定制自己的系统菜单
// WPARAM 参数为点击的鼠标坐标，使用 MAKEWPARAM(pt.x,pt.y)制作而成
#define UM_WM_POPUPSYSTEMMENU						(UM_UI_BASE_DEF_MSG + 0x000C)
