...
public class ModeTollGateSmallMapView extends ViewGroup {
	...
	// 打开通关模式小地图选择界面
	public final static int OPEN_GAME = 1;

	// 不能使用此构造函数进行new操作
	public ModeTollGateSmallMapView(Context context) {
		super(context);
	}

	public ModeTollGateSmallMapView(Context context, Handler handlerObj, int nMapTypeId) {
		super(context);
		...
		// 添加一级地图
		for (int i = 1; i <= 5; i++) {
			ModeTollGateSmallMapViewItem mapItem = new ModeTollGateSmallMapViewItem(
					m_Context);
			if (mapItem != null) {
				mapItem.setMapId(m_nMapTypeId, i);
			...
			// 先过滤多点触摸
				...
				if (nMotion == MotionEvent.ACTION_UP) {
					...
					if (nStepX <= m_nTouchSlop && nStepY <= m_nTouchSlop) {
						...
						// 打开小地图
						openGame(m_nMapTypeId, m_nCurScreen + 1);
						return true;
					}
				}
	...
	private void openGame(int nMapTypeId, int nMapId) {

		if (m_Handler != null && nMapTypeId > 0 && nMapId > 0) {
			Message msg = new Message();
			msg.what = ModeTollGateSmallMapView.OPEN_GAME;
			msg.arg1 = nMapTypeId;
			msg.arg2 = nMapId;
